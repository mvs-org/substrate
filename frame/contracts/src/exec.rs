...
use crate::{
    ...
    rent, storage, ContractInfoOf, Error,
};
    ...
    pub fn call(
        ...
        self.with_nested_context(dest.clone(), dest_trie_id, |nested| {
            ...
            match storage::code_hash::<T>(&dest) {
                ...
                Err(storage::ContractAbsentError) => Ok(ExecReturnValue {
...
mod tests {
    use super::{
        BalanceOf, Event, ExecFeeToken, ExecResult, ExecutionContext, Ext, Loader, RawEvent,
        ReturnFlags, TransferFeeKind, TransferFeeToken, Vm,
    };
            ...
            if !*reached_bottom {
                ...
                assert_matches!(
                    ...
                    Err(DispatchError::Other(
                        "reached maximum depth, cannot make a call"
                    ))
                );
                *reached_bottom = true;
            } else {
